...
const Gamestate: React.FC = () => {
  ...
  const [tilesInHand, setTilesInHand] = useState<(string | null)[]>(
  ...
  >({});
  const [immutableBoardTiles, setImmutableBoardTiles] = useState<
    { [key: string]: string | null }
  >({});
  ...
  const [voteCooldownRemaining, setVoteCooldownRemaining] = useState<
    number | null
  >(null);
      ...
      return new Promise((resolve) => {
        ...
        stompClient.onConnect = () => {
          ...
          stompClient.subscribe(
            `/topic/game_states/users/${localStorage.getItem("userId")}`,
            (message) => {
              ...
              if (responseStatus === "VALIDATION_SUCCESS") {
                ...
                showAlertModal(
                  ...
                  `Validation failed! Reason: ${
                    response.message.toString().substring(16)
                  }`,
                );
              } //vote
                ...
                showDecisionModal(
                  "Vote",
                  "Your opponent wants to end the game here. Do you agree?",
                );
              } else if (responseStatus === "SUCCESS" && action === "NO_VOTE") {
                showAlertModal(
                  "Vote",
                  "Your opponent declined ending the game.",
                );
              } //submit
              else if (
                responseStatus === "SUCCESS" &&
                (action === "SUBMIT" || action === "FETCH_GAME_STATE")
              ) {
                ...
                const updatedHand = currentHand.map((tile) => {
                  if (
                    (tile === "" || tile === null) &&
                    letterIndex < newUserLetters.length
                  ) {
                ...
                if (action === "FETCH_GAME_STATE") {
                  ...
                  const parsedBoardTiles = dictifyMatrix(
                    response.gameState.board,
                  );
                  setBoardTiles(parsedBoardTiles);
                  setImmutableBoardTiles((prev) => ({
                    ...prev,
                    ...parsedBoardTiles,
                  })); // Store immutable tiles
                  setPlayerAtTurn(
                    response.gameState.playerId == gameHost.id
                      ? gameHost
                      : gameGuest,
                  ); // Toggle player at turn
          ...
          stompClient.subscribe(`/topic/game_states/${id}`, (message) => {
            ...
            const playerUsername = moveById === gameHost.id.toString()
              ? gameGuest.username
              : gameHost.username;
            if (action === "SUBMIT" && responseStatus === "SUCCESS") {
              ...
              if (moveById === localStorage.getItem("userId")) {
                showAlertModal(
                  "Points scored",
                  `${playerUsername} ${points[0]}!`,
                );
              } else {
                setTilesInHand(
                  response.gameState.userTiles.map((letter: string) =>
                    `/letters/${letter} Tile 70.jpg`
                  ),
                );
              ...
              setPlayerAtTurn((prev) =>
                prev.id === gameHost.id ? gameGuest : gameHost
              );
              setPlayerPoints((prev) => ({
                ...prev,
                ...response.gameState.playerScores,
              ...
              showAlertModal(
                "Game Over",
                moveById !== localStorage.getItem("userId")
                  ? "Your opponent surrendered."
                  : "You have surrendered.",
              );
              setSurrenderedId(moveById);
            } else if (
              (action === "SKIP" || action === "EXCHANGE") &&
              responseStatus === "SUCCESS"
            ) {
              if (moveById === localStorage.getItem("userId")) {
                showAlertModal(
                  ...
                  action === "SKIP"
                    ...
                    : "Your opponent exchanged some tiles.",
                );
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      const response = await apiService.get<number>(
        `/games/${id}/letters/${letter.toUpperCase()}`,
      );
      ...
      showAlertModal(
        "Error",
        `Failed to retrieve letter count: ${(error as Error).message}`,
      );
    }
  };
  ...
  const exchangeTiles = () => {
    ...
    const exchangeList = [];
    const tilesCharOnly = tilesToExchange.filter((tile): tile is string =>
      tile !== null
    ).map((tileSource) => tileSource[9]);
    ...
    showAlertModal(
      "Exchange",
      `${tilesCharOnly} ${
        tilesCharOnly.length == 1 ? "was" : "were"
      } exchanged.`,
    );
  ...
  const handleVote = () => {
    ...
    if (lastVoteTime && currentTime - lastVoteTime < 5 * 60 * 1000) {
      const remainingTime = Math.ceil(
        (5 * 60 * 1000 - (currentTime - lastVoteTime)) / 1000,
      );
      showAlertModal(
        "Error",
        `You can only vote once every 5 minutes. Please wait ${remainingTime} seconds.`,
      );
      return;
    }
  ...
  const commitWord = () => {
    ...
    const matrix = constructMatrix();
    const nonEmptyTilesInHand: string[] = tilesInHand.filter((
      tile,
    ): tile is string => tile !== null && tile !== "").map((tileSource) =>
      tileSource[9]
    );
  ...
  const handleSurrender = () => {
    ...
    if (!id || !stompClientRef.current) {
      ...
      showAlertModal(
        "Error",
        "Cannot surrender. Game ID or WebSocket connection is missing.",
      );
      return;
    }
    const messageBody: GameState = {
  ...
  const handleGameEnd = () => {
    ...
    if (!id || !stompClientRef.current || !token) {
      ...
      showAlertModal(
        "Error",
        "The game cannot be ended since the game cannot be found or the WebSocket client has an error.",
      );
      return;
    }
    const messageBody: GameState = {
  ...
  const handleReturn = () => {
    ...
    for (const key in boardTilesRef.current) {
      ...
      if (emptyIndex !== -1 && image) {
        ...
        console.warn(
          `No space in hand to return tile from board position ${key}`,
        );
      }
    }
  ...
  const handleTileClick = (col: number, row: number) => {
    ...
    const emptyIndex = tilesInHandRef.current.findIndex((tile) =>
      tile === null
    );
  ...
  const handleHandDrop = (e: React.DragEvent, index: number) => {
    ...
    if (tilesInHand[index] !== null) {
      ...
      if (
        (indIncluded || draggedIncluded) && !(indIncluded && draggedIncluded)
      ) { //XOR?
        if (indIncluded) {
          setSelectedTiles((prev) =>
            prev.map((tile) => tile === index ? parseInt(draggedIndex) : tile)
          );
        } else {
          setSelectedTiles((prev) =>
            prev.map((tile) => tile === parseInt(draggedIndex) ? index : tile)
          );
        }
      }
    } else {
    ...
    if (boardTiles[keyTo] || immutableBoardTiles[keyTo]) {
      ...
      if (draggedIndex !== null && isNaN(draggedCol) && isNaN(draggedRow)) {
        ...
        if (selectedTiles.includes(draggedIndex)) {
          setSelectedTiles((prev) =>
            prev.filter((index) => index !== draggedIndex)
          );
        }
  ...
  useEffect(() => {
    const mutableTiles = Object.keys(boardTiles).filter((key) =>
      !immutableBoardTiles[key]
    );
      ...
      <div id="rest-container">
            ...
            <div id="end_vote">
              ...
              >
                {voteCooldownRemaining !== null && voteCooldownRemaining > 0
                  ? `Vote in ${
                    Math.max(0, Math.floor(voteCooldownRemaining / 1000))
                  }s`
                  : "Vote to end"}
              </button>
            </div>
              ...
              <div className="player-container" id="left-player">
                <div className="name-and-dot-container">
                  ...
                  <div className="dot-container">
                    <div
                      className={`turn-dot ${
                        playerAtTurn === gameHost ? "active-dot" : ""
                      }`}
                    >
                    </div>
                  </div>
              ...
              <div className="player-container">
                <div className="name-and-dot-container">
                  ...
                  <div className="dot-container">
                    <div
                      className={`turn-dot ${
                        playerAtTurn === gameGuest ? "active-dot" : ""
                      }`}
                    >
                    </div>
                  </div>
        ...
        <div id="bag-stuff">
          <div id="bag-image">
            <Image
              id="bag-jpg"
              src="/BagWithTiles.png"
              alt="Letters Bag"
              width={222}
              height={168}
              priority
            />{" "}
            {/* without tiles: /TilesBag.png*/}
          </div>
          <div id="bag-info">
        ...
        <div id="game-buttons">
          <div id="upper-row-container">
            <div id="verify-button-container">
              <Button
                ...
                title={!isTileOnBoard
                  ? "Place a tile on the board to verify a word"
                  : ""}
              >
                Verify
              </Button>
          ...
          <CustomAlertModal
            ...
            onClose={alertModalTitle === "Game Over"
              ? handleGameOverClose
              : handleModalClose}
          />
          <CustomDecisionModal
            ...
            onConfirm={decisionModalTitle === "Vote"
              ? handleGameEnd
              : handleSurrender}
            onCancel={decisionModalTitle === "Vote"
              ? handleDecline
              : handleModalClose}
          />
          <CustomAlertModal
            ...
            message={
              <div style={{ fontSize: "1.1em", padding: "0.5em 0" }}>
                ...
                <ul className="rules-feature-list">
                  <li>
                    <b>Verify</b>: Check if the word you formed is valid before
                    playing it. This does not commit your move.
                  </li>
                  <li>
                    <b>Exchange</b>: Select one or more tiles in your hand and
                    exchange them for new ones from the bag.
                  </li>
                  <li>
                    <b>Skip</b>: Skip your turn without making a move. Useful if
                    you can&apos;t play or want to wait for a better
                    opportunity.
                  </li>
                  <li>
                    <b>Play Word</b>: Commit your verified word to the board and
                    end your turn. Only enabled after a successful verification.
                  </li>
                  <li>
                    <b>Vote to end</b>: Propose to end the game early. If both
                    players agree, the game ends.
                  </li>
                  <li>
                    <b>Give Up</b>: Surrender the game. Your opponent will be
                    declared the winner.
                  </li>
                  <li>
                    <b>Ask</b>: Enter a letter and click &quot;Ask&quot; to see
                    how many tiles of that letter remain in the bag.
                  </li>
                </ul>
              </div>
            }

    ...
    apiService.get<Game>(`/games/${id}`)
      .then((game) => {
        ...
        if (game.surrenderId !== null) {
          ...
          if (
            game.users[0].id.toString() == localStorage.getItem("SurrenderedId")
          ) {
          ...
          if (
            playerPoints[game.users[0].id] !== null &&
            playerPoints[game.users[0].id] !== undefined &&
            playerPoints[game.users[1].id] !== null &&
            playerPoints[game.users[1].id] !== undefined
          ) {
            if (
              playerPoints[game.users[0].id]! > playerPoints[game.users[1].id]!
            ) {
              setWinner(game.users[0]);
              setLoser(game.users[1]);
            } else {
      ...
      <main>
        ...
        <div id="left_side">
          <div id="end_reason_container">
            <span id="end_reason">
              The game ended with {surrendered ? "somebody" : "nobody"}{" "}
              surrendering.
            </span>
          </div>
